...
export default function InstructionsPanel({
  ...
  const zoneName = (id: string) => zoneList.find((z) => z.id === id)?.name || t("エリア");

  const markRead = trpc.genba.instructions.markRead.useMutation();
  const create = trpc.genba.instructions.create.useMutation({
    onSuccess: () => { utils.genba.instructions.listForMe.invalidate({ siteId }); setText(""); setZoneId(""); toast.success(t("指示を送信しました")); onReadChanged(); },
  ...
  const targetLabel = (i: any) => {
    if (i.targetKind === "all") return t("全員");
    if (i.targetKind === "team") return teamList.find((tm) => tm.id === i.targetId)?.name || t("班");
    return userName(Number(i.targetId));
  };

  function send() {
    const txt = text.trim();
    if (!txt) { toast.error(t("指示の内容を入力してください")); return; }
    ...
    create.mutate({ siteId, text: txt, targetKind, targetId, zoneId: zoneId || undefined });
  }

  const inner = (
      <>
        {!embedded && <DialogHeader><DialogTitle>📣 {t("指示")}</DialogTitle></DialogHeader>}

        {canEdit && (
          <div className="rounded-lg border border-border p-2 space-y-2">
            <textarea value={text} onChange={(e) => setText(e.target.value)} rows={3}
              placeholder={t("例: 本日中に1-1エリアの建て込みを完了してください")}
            ...
            <div className="grid grid-cols-2 gap-2">
              <select value={target} onChange={(e) => setTarget(e.target.value)} className="rounded-md border border-border bg-background p-2 text-sm">
                <option value="all">👥 {t("全員へ")}</option>
                {teamList.map((g) => <option key={g.id} value={`team:${g.id}`}>🏳 {g.name}（{g.memberIds.length}{t("名")}）{t("へ")}</option>)}
                {userList.map((u) => <option key={u.id} value={`worker:${u.id}`}>👤 {u.name || `user#${u.id}`}{t("へ")}</option>)}
              </select>
              ...
              {zoneId ? (
                <div className="flex items-center gap-1 rounded-md border border-[#005AFF]/40 bg-[#005AFF]/5 p-1 pl-2 text-sm">
                  ...
                  <button type="button" onClick={() => setShowZonePicker(true)} className="text-[11px] px-1.5 py-0.5 rounded border border-border text-muted-foreground hover:text-foreground">{t("変更")}</button>
                  <button type="button" title={t("エリア指定を外す")} onClick={() => setZoneId("")} className="text-muted-foreground hover:text-destructive px-1"><X className="h-3.5 w-3.5" /></button>
                </div>
              ) : (
                <button type="button" onClick={() => setShowZonePicker(true)}
                  ...
                  title={t("この指示が対象とするエリア（工区）を図から選ぶ")}>
                  <MapPin className="h-4 w-4" /> {t("図からエリアを選択")}
                </button>
              )}
            </div>
            <Button className="w-full" onClick={send} disabled={create.isPending}>{t("送信")}</Button>
          </div>
        )}

        <div className="space-y-2">
          {items.length === 0 && <p className="text-sm text-muted-foreground py-2">{t("指示はまだありません。")}</p>}
          {items.map((i) => {
            const unread = i.mine && !i.read;
            const targetUsers = i.targetKind === "team" ? (teamList.find((tm) => tm.id === i.targetId)?.memberIds || []) : i.targetKind === "worker" ? [Number(i.targetId)] : userList.map((u) => u.id);
            ...
            return (
              <div key={i.id} className="rounded-lg border border-border p-2" style={{ borderLeft: unread ? "4px solid #FF4B00" : undefined }}>
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  ...
                  {unread && <span className="text-[11px] px-1.5 py-0.5 rounded bg-[#FF4B00] text-white font-bold">{t("未読")}</span>}
                </div>
                <div className="text-sm whitespace-pre-wrap">{i.text}</div>
                {canEdit && <div className="text-xs text-muted-foreground mt-1">{t("既読")} {readCount}/{targetUsers.length}</div>}
              </div>
            );
          })}
        </div>
