...
function NewReport({ floorId, x, y, zoneId, onClose, onSaved }: { floorId: string; x: number; y: number; zoneId: string | null; onClose: () => void; onSaved: () => void }) {
  ...
  const create = trpc.genba.floors.pins.create.useMutation({
    onSuccess: () => { toast.success(t("問題を報告しました")); onSaved(); },
  ...
  async function onPhotoChosen(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      toast.error(err?.message || t("写真の処理に失敗しました"));
    } finally { setBusy(false); }
  }

  function submit() {
    if (!text.trim() && photos.length === 0) { toast.error(t("コメントか写真を入れてください")); return; }
  ...
  return (
    <Dialog open onOpenChange={(v) => { if (!v) onClose(); }}>
      <DialogContent className="max-h-[85vh] overflow-y-auto">
        <DialogHeader><DialogTitle className="text-sm">⚠ {t("この位置の問題を報告")}</DialogTitle></DialogHeader>
        <div className="space-y-3">
          <textarea value={text} onChange={(e) => setText(e.target.value)} rows={3}
            placeholder={t("例: この柱まわりのスリーブ位置が図面と相違")}
          ...
          <div className="space-y-2">
            ...
            <Button type="button" variant="outline" size="sm" disabled={busy || photos.length >= 4} onClick={() => fileRef.current?.click()}>
              ...
              {t("写真を追加")}（{photos.length}/4）
            </Button>
            {photos.length > 0 && (
              <div className="flex gap-2 flex-wrap">
                {photos.map((p, i) => (
                  <div key={i} className="relative">
                    <img src={`data:${p.mimeType};base64,${p.base64}`} alt={`${t("写真")}${i + 1}`} className="h-16 w-16 rounded object-cover border border-border" />
          ...
          <div className="flex gap-2 pt-1">
            <Button variant="ghost" size="sm" className="flex-1" onClick={onClose}>{t("キャンセル")}</Button>
            <Button size="sm" className="flex-1" disabled={create.isPending} onClick={submit}>
              {create.isPending && <Loader2 className="h-4 w-4 mr-1 animate-spin" />}{t("報告する")}
            </Button>
          </div>
        </div>
      </DialogContent>
...
function PinDetail({ pin, canEdit, onClose, onChanged }: { pin: MapPin; canEdit: boolean; onClose: () => void; onChanged: () => void }) {
  ...
  const resolve = trpc.genba.floors.pins.resolve.useMutation({
    onSuccess: () => { toast.success(resolved ? t("未解決に戻しました") : t("解決済みにしました")); onChanged(); },
    onError: (e) => toast.error(e.message),
  });
  const remove = trpc.genba.floors.pins.remove.useMutation({
    onSuccess: () => { toast.success(t("報告を削除しました")); onChanged(); onClose(); },
    onError: (e) => toast.error(e.message),
  });
        ...
        <DialogHeader>
          <DialogTitle className="text-sm flex items-center gap-2">
            ...
            <span className={`text-[10px] px-1.5 py-0.5 rounded ${resolved ? "bg-[#03AF7A]/15 text-[#03AF7A]" : "bg-[#FF4B00]/15 text-[#FF4B00]"}`}>
              {resolved ? t("解決済み") : t("未解決")}
            </span>
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          <div className="text-sm whitespace-pre-wrap">{pin.text || t("(コメントなし)")}</div>
          {pin.photoUrls.length > 0 && (
            <div className="flex gap-2 flex-wrap">
              {pin.photoUrls.map((u, i) => (
                <a key={i} href={u} target="_blank" rel="noopener noreferrer">
                  <img src={u} alt={`${t("写真")}${i + 1}`} className="h-24 w-24 rounded object-cover border border-border" />
                </a>
              ))}
            </div>
          )}
          <div className="text-[11px] text-muted-foreground">{t("報告者")}: {pin.byUserName || t("不明")}</div>
          ...
          {canEdit && (
            <div className="flex gap-2 pt-1 border-t border-border">
              <Button variant="outline" size="sm" className="flex-1" disabled={resolve.isPending}
                ...
                {resolved ? t("未解決に戻す") : t("解決済みにする")}
              </Button>
              <Button variant="ghost" size="sm" className="text-destructive" disabled={remove.isPending}
                onClick={() => { if (window.confirm(t("この報告を削除しますか？"))) remove.mutate({ id: pin.id }); }}>
