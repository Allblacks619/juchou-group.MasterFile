...
export default function MaterialsPanel({
  ...
  const cancel = trpc.genba.materials.cancelRequest.useMutation({
    onSuccess: () => { invalidate(); toast.success(t("依頼を取り消しました")); }, onError: (e) => toast.error(e.message),
  });
  const savePreset = trpc.genba.materials.savePreset.useMutation({
    onSuccess: () => { utils.genba.materials.listPresets.invalidate(); toast.success(t("プリセットを保存しました")); },
    onError: (e) => toast.error(e.message),
  });
  ...
  function addToCart(name: string, u?: string) {
    ...
    if (!nm) { toast.error(t("材料名を選択または入力してください")); return; }
  ...
  function send() {
    if (cart.length === 0) { toast.error(t("材料をリストに追加してください")); return; }
    create.mutate({ siteId, note: note.trim() || undefined, items: cart });
  }
  function saveCartAsPreset() {
    if (cart.length === 0) { toast.error(t("リストが空です")); return; }
    const name = window.prompt(t("プリセット名 (工事名など)"), t("よく使う材料"));
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] overflow-y-auto">
        <DialogHeader><DialogTitle>{t("📦 材料発注")}</DialogTitle></DialogHeader>

        {canEdit && (
          <div className="flex gap-2">
            <button onClick={() => setView("list")} className={`px-3 py-1.5 rounded-lg text-sm border ${view === "list" ? "bg-gold/10 text-gold border-gold/40" : "border-border text-muted-foreground"}`}>{t("📋 依頼一覧")}</button>
            <button onClick={() => setView("agg")} className={`px-3 py-1.5 rounded-lg text-sm border ${view === "agg" ? "bg-gold/10 text-gold border-gold/40" : "border-border text-muted-foreground"}`}>{t("Σ 集計(発注用)")}</button>
          </div>
        )}

        {canEdit && view === "agg" ? (
          <div className="rounded-lg border border-border p-3 space-y-2">
            <strong className="text-sm">{t("Σ 材料の必要数 集計")}</strong>
            <div className="flex gap-2 flex-wrap items-center">
              {([["today", "今日"], ["week", "今週"], ["all", "全期間"]] as const).map(([k, label]) => (
                <button key={k} onClick={() => setAggPeriod(k)} className={`px-2.5 py-1 rounded-md text-xs border ${aggPeriod === k ? "bg-gold/10 text-gold border-gold/40" : "border-border text-muted-foreground"}`}>{t(label)}</button>
              ))}
              <label className="ml-auto text-xs flex items-center gap-1 cursor-pointer">
                <input type="checkbox" checked={aggPendingOnly} onChange={(e) => setAggPendingOnly(e.target.checked)} /> {t("依頼中のみ")}
              </label>
            </div>
            <p className="text-[11px] text-muted-foreground">{t("依頼日ベースで集計（今週=月曜起点）。このまま上位への発注リストに使えます。")}</p>
            {aggRows.length === 0 && <p className="text-sm text-muted-foreground py-2">{t("該当する依頼はありません。")}</p>}
            {aggRows.map((row, i) => (
              <div key={i} className="flex items-center gap-2 py-1.5 border-b border-border/50">
                ...
                <span className="text-[11px] text-muted-foreground">{row.count}{t("件")}</span>
          ...
          <>
            {/* 依頼作成 */}
            <div className="rounded-lg border border-border p-3 space-y-2">
              <strong className="text-sm">{t("📦 材料の発注を依頼する")}</strong>
              <div className="text-xs text-muted-foreground">{t("カタログから選択（未来工業・ネグロス電工 他）")}</div>
              <div className="flex gap-2 flex-wrap">
                ...
                <select value={presetPart} onChange={(e) => pickPresetPart(e.target.value)} disabled={!group} className="flex-[1.4] min-w-[150px] rounded-md border border-border bg-background p-2 text-sm disabled:opacity-50">
                  <option value="">{t("材料を選択")}</option>
                  {group?.parts.map((pt, i) => <option key={i} value={pt.label}>{dispName(pt.label)}</option>)}
                </select>
              </div>

              <div className="text-xs text-muted-foreground">{t("または直接入力（型番の一部で候補が出ます）")}</div>
              <div className="relative">
                <input value={freeName} onChange={(e) => { setFreeName(e.target.value); setPresetPart(""); }}
                  placeholder={t("材料名・型番（例: D1、VVF、ビニテ）")} className="w-full rounded-md border border-border bg-background p-2 text-sm" />
              ...
              <div className="flex gap-2 items-center flex-wrap">
                <span className="text-sm text-muted-foreground">{t("個数")}</span>
                ...
                <Button size="sm" className="ml-auto" onClick={() => addToCart(presetPart || freeName)}>{t("＋ リストに追加")}</Button>
              </div>
              ...
              {cart.length > 0 && (
                <div className="rounded-md border border-border p-2 space-y-1">
                  ...
                  {canEdit && <button className="text-xs text-muted-foreground hover:text-gold mt-1" onClick={saveCartAsPreset}>{t("★ このリストをプリセット保存")}</button>}
                </div>
              )}
              <input value={note} onChange={(e) => setNote(e.target.value)}
                placeholder={t("ひとこと（例: 1-1エリアの建て込み分）")} className="w-full rounded-md border border-border bg-background p-2 text-sm" />
              <Button className="w-full" onClick={send} disabled={create.isPending}>{t("📦 材料の発注を依頼する")}（{cart.length}）</Button>
            </div>

            {/* 依頼一覧 */}
            {requests.length === 0 && <p className="text-sm text-muted-foreground py-2">{t("発注依頼はまだありません。")}</p>}
            {requests.map((r) => {
              ...
              return (
                <div key={r.id} className="rounded-lg border border-border p-2" style={{ borderLeft: `4px solid ${st.color}` }}>
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    <span className="text-[11px] px-1.5 py-0.5 rounded" style={{ background: st.color, color: st.text }}>{t(st.label)}</span>
                  ...
                  <div className="flex gap-2 mt-2">
                    {canEdit && r.status === "pending" && (
                      <Button size="sm" className="flex-1" onClick={() => setStatus.mutate({ id: r.id, status: "ordered" })}>{t("📦 発注済にする")}</Button>
                    )}
                    {canEdit && r.status === "ordered" && (
                      <Button size="sm" className="flex-1" style={{ background: "#03AF7A" }} onClick={() => setStatus.mutate({ id: r.id, status: "delivered" })}>{t("✅ 納品済にする")}</Button>
                    )}
                    {mine && r.status === "pending" && !canEdit && (
                      <Button size="sm" variant="outline" className="flex-1" onClick={() => { if (window.confirm(t("この依頼を取り消しますか？"))) cancel.mutate({ id: r.id }); }}>{t("依頼を取り消す")}</Button>
                    )}
                  </div>
                </div>
              );
