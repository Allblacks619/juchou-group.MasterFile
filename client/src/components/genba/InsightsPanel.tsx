...
export default function InsightsPanel({
  ...
  const promote = trpc.genba.materials.savePreset.useMutation({
    onSuccess: () => { utils.genba.logs.insights.invalidate({ siteId }); utils.genba.materials.listPresets.invalidate(); toast.success("プリセットに追加しました"); },
  ...
  function addPreset(name: string) {
    ...
    if (existing) {
      if ((existing.parts || []).includes(name)) { toast.info("すでに登録済みです"); return; }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] overflow-y-auto">
        <DialogHeader><DialogTitle>📈 学習と改善提案{total > 0 ? `（${total}）` : ""}</DialogTitle></DialogHeader>
        <p className="text-xs text-muted-foreground">アプリの使われ方から自動で提案します。</p>

        {/* 改善提案 */}
        <div className="text-sm font-bold text-[#7c3aed]">💡 改善のヒント</div>
        {total === 0 && <p className="text-sm text-muted-foreground py-2">提案はありません。使い込むほど提案が増えます 👍</p>}
        ...
        {(d?.unusedTemplates?.length ?? 0) > 0 && (
          <div className="rounded-lg border border-border p-2">
            <div className="text-sm font-semibold">🧹 未使用の作業テンプレート</div>
            <div className="text-xs text-muted-foreground">{d!.unusedTemplates.slice(0, 8).join("、")}{d!.unusedTemplates.length > 8 ? " 他" : ""} — 作業テンプレートから削除できます</div>
          </div>
        )}

        {/* 統計 */}
        <div className="text-sm font-bold text-muted-foreground mt-2">📊 利用統計</div>
        ...
        {(d?.topMaterials?.length ?? 0) > 0 && (
          <div>
            <div className="text-xs font-bold text-muted-foreground mb-1">よく発注される材料 TOP5</div>
        ...
        <p className="text-[11px] text-muted-foreground mt-2">※ ログは改善のための集計用です（直近1000件）。</p>
      </DialogContent>
    </Dialog>
  );
}
