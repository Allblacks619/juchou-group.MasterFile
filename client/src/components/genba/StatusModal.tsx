...
export default function StatusModal({
  ...
  async function submit(p: SetStatusPayload) {
    ...
    try {
      ...
      toast.error(e?.message || t("更新に失敗しました"));
    } finally {
      setBusy(false);
    }
  }
  ...
  async function onPhotoChosen(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    for (const f of files.slice(0, 4 - photos.length)) {
      ...
      } catch { toast.error(t("写真の読み込みに失敗しました")); }
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{t("進捗を登録")}: {dispName(task.name, (task as any).romaji)}</DialogTitle>
        </DialogHeader>

        {progressOpen ? (
          <div className="space-y-3">
            <div className="text-sm font-bold text-[#0369a1]">{t("▶ 進捗はどれくらいですか？")}</div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              {photos.map((p, i) => (
                <div key={i} className="relative">
                  <img src={p.preview} alt={`${t("添付")}${i + 1}`} className="h-16 w-16 rounded object-cover border border-border" />
              ...
              {photos.length < 4 && (
                <button className="h-16 w-16 rounded border border-dashed border-border flex flex-col items-center justify-center text-xs text-muted-foreground"
                  ...
                  <Camera className="h-4 w-4 mb-0.5" />{t("写真")}
                </button>
              )}
              <input ref={photoRef} type="file" accept="image/*" capture="environment" multiple className="hidden" onChange={onPhotoChosen} />
            </div>
            <div className="flex gap-2">
              <Button variant="outline" className="flex-1" onClick={() => setIssueOpen(false)}>{t("◀ 戻る")}</Button>
              <Button className="flex-[2]" disabled={busy} style={{ background: STATUS.issue.color }}
                ...
                {busy && <Loader2 className="h-4 w-4 mr-1 animate-spin" />}{t("⚠ 問題を報告する")}
              </Button>
            </div>
          </div>
        )}
