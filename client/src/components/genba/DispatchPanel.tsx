...
export default function DispatchPanel({
  ...
  const create = trpc.genba.dispatches.create.useMutation({
    onSuccess: () => { invalidate(); setTaskId(""); setMemo(""); setPicked([]); toast.success(t("急ぎ手配を送信しました") + " ⚡"); },
    onError: (e) => toast.error(e.message),
  });
  const setDone = trpc.genba.dispatches.setDone.useMutation({ onSuccess: invalidate, onError: (e) => toast.error(e.message) });
  const remove = trpc.genba.dispatches.remove.useMutation({ onSuccess: () => { invalidate(); toast.success(t("手配を削除しました")); }, onError: (e) => toast.error(e.message) });

  const togglePick = (id: number) => setPicked((p) => (p.includes(id) ? p.filter((x) => x !== id) : [...p, id]));
  function submit() {
    if (!zoneId) { toast.error(t("エリアを選択してください")); return; }
    if (!taskId) { toast.error(t("作業を選択してください")); return; }
    if (picked.length === 0) { toast.error(t("作業員を1名以上選択してください")); return; }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] overflow-y-auto">
        <DialogHeader><DialogTitle>⚡ {t("今日の急ぎ手配")}</DialogTitle></DialogHeader>

        {/* 作成フォーム (field) */}
        {canEdit && (
          <div className="rounded-lg border border-border p-3 space-y-2">
            <strong className="text-sm">⚡ {t("急ぎの作業を手配する")}</strong>
            <div className="grid grid-cols-2 gap-2">
              ...
              <select value={taskId} onChange={(e) => setTaskId(e.target.value)} disabled={!zoneId} className="rounded-md border border-border bg-background p-2 text-sm disabled:opacity-50">
                <option value="">{t("作業を選択")}</option>
                {taskList.map((t) => <option key={t.id} value={t.id}>{dispName(t.name, (t as any).romaji)}</option>)}
              </select>
            </div>

            <div>
              <div className="text-xs text-muted-foreground mb-1">{t("作業員（複数可）")}</div>
              <div className="flex flex-wrap gap-1.5">
                {userList.length === 0 && <span className="text-[11px] text-muted-foreground">{t("割当可能な作業員がいません（案件連携中は出面登録者のみ）。")}</span>}
                {userList.map((u) => {
            ...
            <div className="flex items-center gap-2">
              <label className="text-xs text-muted-foreground">{t("対象日")}</label>
            ...
            <input value={memo} onChange={(e) => setMemo(e.target.value)} placeholder={t("メモ（例: 12時までに完了。資材は倉庫前）")} className="w-full rounded-md border border-border bg-background p-2 text-sm" />
            <Button className="w-full" onClick={submit} disabled={create.isPending}>⚡ {t("この作業を手配する")}（{picked.length}{t("名")}）</Button>
          </div>
        )}

        {/* 手配ボード */}
        <div className="flex gap-2">
          <button onClick={() => setScope("today")} className={`px-3 py-1.5 rounded-lg text-sm border ${scope === "today" ? "bg-gold/10 text-gold border-gold/40" : "border-border text-muted-foreground"}`}>{t("今日")}</button>
          <button onClick={() => setScope("all")} className={`px-3 py-1.5 rounded-lg text-sm border ${scope === "all" ? "bg-gold/10 text-gold border-gold/40" : "border-border text-muted-foreground"}`}>{t("すべて")}</button>
        </div>

        <div className="space-y-2">
          {sorted.length === 0 && <p className="text-sm text-muted-foreground py-2">{scope === "today" ? t("今日の急ぎ手配はありません。") : t("急ぎ手配はまだありません。")}</p>}
          {sorted.map((d) => {
            ...
            return (
              <div key={d.id} className={`rounded-lg border p-2 ${d.done ? "opacity-60" : ""}`}
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-[11px] px-1.5 py-0.5 rounded bg-[#FF4B00] text-white font-bold">⚡ {t("急ぎ")}</span>
                  ...
                  {d.done && <span className="text-[11px] px-1.5 py-0.5 rounded bg-[#03AF7A] text-white">{t("対応済")}</span>}
                </div>
                ...
                <div className="flex gap-2 mt-2">
                  {canDone && (
                    <Button size="sm" variant={d.done ? "outline" : "default"} className="flex-1" style={d.done ? undefined : { background: "#03AF7A" }}
                      ...
                      <Check className="h-4 w-4 mr-1" />{d.done ? t("未対応に戻す") : t("対応済にする")}
                    </Button>
                  )}
                  {canEdit && (
                    <button onClick={() => { if (window.confirm(t("この手配を削除しますか？"))) remove.mutate({ id: d.id }); }} className="text-muted-foreground hover:text-destructive px-2" title={t("削除")}>
