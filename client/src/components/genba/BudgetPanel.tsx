...
export default function BudgetPanel({
  ...
  const save = trpc.genba.budgets.save.useMutation({ onSuccess: () => { invalidate(); toast.success(t("予算設定を保存しました")); }, onError: (e) => toast.error(e.message) });
  const addAtt = trpc.genba.budgets.addManualAttendance.useMutation({ onSuccess: () => { invalidate(); setAttMd(""); toast.success(t("出面を記録しました")); }, onError: (e) => toast.error(e.message) });
  ...
  function disable() {
    if (window.confirm(t("この現場の予算トラッカーを無効化しますか？\n（設定と出面記録は保持され、再有効化で戻ります）"))) save.mutate({ siteId, enabled: false });
  }
  function addAttendance() {
    const md = Number(attMd);
    if (!attDate || !md || md <= 0) { toast.error(t("日付と人工数を入力してください")); return; }
  ...
  const numField = (label: string, key: keyof BudgetForm, suffix = "円") => (
    <label className="flex items-center gap-2 text-sm mt-2">
      <span className="min-w-[110px] text-muted-foreground">{t(label)}</span>
      ...
      <span className="text-xs text-muted-foreground w-7">{t(suffix)}</span>
    </label>
  );
  ...
  const inner = (
      <>
        {!embedded && <DialogHeader><DialogTitle>{t("💰 予算トラッカー")}</DialogTitle></DialogHeader>}
        ...
        {!enabled ? (
          <div className="rounded-lg border border-border p-4 text-center space-y-3">
            ...
            <div className="text-sm text-muted-foreground text-left leading-relaxed">
              {t("工期・契約金額・人工単価から「あと何人工使えるか」を自動計算します。")}<br />
              <strong>{t("常駐現場（出勤分だけ請求する現場）など逆算が不要な場合は、このままでOK")}</strong>{t("。未設定でも他機能に影響はありません。")}
            </div>
            <Button className="w-full" onClick={enable} disabled={save.isPending}>{t("この現場で予算トラッカーを使う")}</Button>
          </div>
        ) : (
          <>
            {/* サマリー */}
            {calc && (
              <div className="rounded-lg border border-border p-3 space-y-3">
                <div className="text-xs text-muted-foreground">{siteName} — {t("逆算サマリー")}（{t("出面")}: {summary?.source === "project" ? t("出面表連携") : t("手入力")} / {t("使用人工")} {round1(calc.usedManDays)}）{summary?.periodFromProject && <span className="text-[#005AFF]">{t("・工期は連携案件から自動取得")}</span>}</div>
                <div className="flex gap-2 flex-wrap">
                  <div className="flex-1 min-w-[140px] rounded-lg border border-border p-2 text-center">
                    ...
                    <div className="text-[11px] text-muted-foreground">{t("残り予算（利益確保後）")}</div>
                  </div>
                  <div className="flex-1 min-w-[140px] rounded-lg border border-border p-2 text-center">
                    <div className="text-xl font-bold tabular-nums">{fmtYen(calc.usedTotal)}</div>
                    <div className="text-[11px] text-muted-foreground">{t("使用済み（人工+経費）")}</div>
                  </div>
                </div>
                {/* 工期 vs 予算 */}
                <div>
                  <div className="flex justify-between text-xs text-muted-foreground">
                    <span>{t("工期消化")} {Math.round(calc.periodPct)}%</span>
                    <span style={{ color: calc.budgetPct > calc.periodPct + 5 ? "#FF4B00" : undefined, fontWeight: calc.budgetPct > calc.periodPct + 5 ? 700 : 400 }}>
                      {t("予算消化")} {Math.round(calc.budgetPct)}%{calc.budgetPct > calc.periodPct + 5 ? " ⚠" : ""}
                    </span>
                  </div>
                ...
                <div className="flex gap-2 flex-wrap">
                  <div className="flex-1 min-w-[140px] rounded-lg border border-border p-2 text-center bg-[#eff6ff] dark:bg-transparent">
                    ...
                    <div className="text-[11px] text-muted-foreground">{t("使用できる人工の上限（月平均）")}</div>
                  </div>
                  <div className="flex-1 min-w-[120px] rounded-lg border border-border p-2 text-center">
                    ...
                    <div className="text-[11px] text-muted-foreground">{t("現在ペース（人工/月）")}</div>
                  </div>
                </div>
                <div className="text-[11px] text-muted-foreground leading-relaxed">
                  {t("予算上限")} {fmtYen(calc.budgetCap)}（{t("契約")} {fmtYen(form.contractAmount)} − {t("目標利益")} {fmtYen(calc.targetProfit)}）／ {t("残工期")} {round1(calc.remainingMonths)} {t("ヶ月")} ／ {t("使用可能な残り人工 約")}{Math.max(Math.floor(calc.allowableManDays), 0)} {t("人工")}
                </div>
              </div>
            )}
            {!calc && (() => {
              ...
              if (!form.contractAmount) return <p className="text-xs text-muted-foreground">{t("契約金額を入力すると逆算サマリーが表示されます。")}</p>;
              if (!effStart || !effEnd) {
                const miss = !effStart && !effEnd ? "工期の開始日と終了日" : !effEnd ? "工期の終了日" : "工期の開始日";
                return (
                  <p className="text-xs text-[#b45309] leading-relaxed">
                    {t("逆算には工期（開始日〜終了日）が必要です。")}<strong>{t(miss)}</strong>{t("が未入力のため計算できません。上の工期欄に入力して「保存」してください。")}
                    {linked && t("（連携案件に工期が設定されていれば自動で使われます）")}
                  </p>
                );
              }
              return <p className="text-xs text-muted-foreground">{t("設定を「保存」すると逆算サマリーが表示されます。")}</p>;
            })()}

            {/* 設定 */}
            <div className="rounded-lg border border-border p-3">
              <div className="flex items-center">
                <strong className="flex-1 text-sm">⚙ {t("プロジェクト設定")}</strong>
                <button className="text-[11px] text-muted-foreground hover:text-[#FF4B00]" onClick={disable}>{t("この現場では使わない")}</button>
              </div>
              <div className="flex items-center gap-2 text-sm mt-2">
                <span className="min-w-[110px] text-muted-foreground">{t("工期")}</span>
              ...
              <div className="flex items-center gap-2 text-sm mt-2">
                <span className="min-w-[110px] text-muted-foreground">{t("目標利益")}</span>
                <select value={form.targetType} onChange={(e) => set({ targetType: e.target.value as any })} className="w-[70px] rounded-md border border-border bg-background p-1.5">
                  ...
                  <option value="amount">{t("円")}</option>
                </select>
              ...
              <label className="flex items-center gap-2 text-sm mt-2">
                <span className="min-w-[110px] text-muted-foreground">{t("連携する出面表")}</span>
                <select value={form.attendanceSource} onChange={(e) => set({ attendanceSource: e.target.value as any })} className="flex-1 rounded-md border border-border bg-background p-1.5">
                  <option value="manual">{t("手入力")}</option>
                  <option value="project">{t("業務システムの出面表")}{got?.projectId ? t("（連携中）") : t("（現場に案件リンク未設定）")}</option>
                </select>
              </label>
              {form.attendanceSource === "project" && !got?.projectId && (
                <div className="text-[11px] text-[#b45309] mt-1 ml-[118px]">{t("この現場に案件（projectId）が未リンクのため、手入力が使われます。")}</div>
              )}
              <Button className="w-full mt-3" onClick={persist} disabled={save.isPending}>{t("保存")}</Button>
            </div>

            {/* 手入力出面 */}
            {form.attendanceSource === "manual" && (
              <div className="rounded-lg border border-border p-3">
                <strong className="text-sm">👷 {t("出面（人工）を記録")}</strong>
                <div className="flex gap-2 mt-2 flex-wrap">
                  ...
                  <input type="number" step="0.5" min="0.5" value={attMd} onChange={(e) => setAttMd(e.target.value)} placeholder={t("人工数(例: 6)")} className="w-[120px] text-right rounded-md border border-border bg-background p-1.5" />
                  <Button size="sm" onClick={addAttendance} disabled={addAtt.isPending}>{t("登録")}</Button>
                </div>
                {rows.length > 0 && (
                  <div className="mt-2">
                    {[...rows].reverse().slice(0, 10).map((a) => (
                      <div key={a.id} className="flex items-center gap-2 py-1.5 border-b border-border/40 text-sm">
                        ...
                        <strong className="flex-1 tabular-nums">{a.manDays} {t("人工")}</strong>
                        <button className="text-[#FF4B00]" onClick={() => { if (window.confirm(`${a.date}${t(" の ")}${a.manDays}${t("人工を削除しますか？")}`)) rmAtt.mutate({ id: a.id }); }}>✕</button>
                      </div>
                    ))}
                    <div className="text-xs text-muted-foreground mt-1">{t("記録合計")}: {round1(rows.reduce((s, r) => s + r.manDays, 0))} {t("人工")}（+ {t("導入前")} {form.preManDays || 0} {t("人工")}）</div>
                  </div>
                )}
              </div>
            )}
