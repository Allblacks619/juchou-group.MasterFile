...
export default function ConnectorPickTool({ lang }: { lang: GenbaLang }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-base font-bold">{t("接続材 選定")}</h2>
      </div>
      ...
      {tab === "t" && (
        <TTab lang={lang} main={tMain} branch={tBranch} onMain={setTMain} onBranch={setTBranch} />
      )}
      {tab === "p" && (
        <CounterTab
          lang={lang}
          ...
          cols={5}
          renderTotalBar={(total) => `${t("断面積合計")} ${total.toFixed(2)} mm²`}
          renderResult={(total) => <PResult lang={lang} total={total} />}
          notes={[t("※ ニチフ P形裸圧着スリーブカタログ準拠。")]}
        />
      )}
      {tab === "r" && (
        <CounterTab
          ...
          renderTotalBar={(_total, counts) => {
            ...
            return text === "" ? t("電線の合計") : `${t("電線の合計")}: ${text}`;
          }}
          renderResult={(_total, counts) => (
            <RingResult lang={lang} n16={counts[0] ?? 0} n20={counts[1] ?? 0} n26={counts[2] ?? 0} />
          )}
          notes={[
            t("※ 内線規程・JIS C 2806準拠。1.6mm・2.0mm・2.6mm単線用。"),
            t("※ より線・CV等は断面積に応じて別途確認してください。"),
          ]}
        />
      )}
      {tab === "ce" && (
        <CounterTab
          ...
          cols={4}
          renderTotalBar={(total) => `${t("断面積合計")} ${total.toFixed(2)} mm²`}
          renderResult={(total) => <CEResult lang={lang} total={total} />}
          notes={[
            t("※ ニチフ CE形裸圧着スリーブカタログ準拠。"),
            t("※ 断面積合計から号数を目安として表示します。必ずカタログで確認してください。"),
          ]}
        />
      )}
      ...
      <div className="text-[11px] text-muted-foreground px-1 space-y-0.5">
        <p>{t("※ メーカーカタログ・規格に基づく目安値です。")}</p>
        <p>{t("※ 実施工では現場実測・設計図書・メーカーカタログを優先してください。")}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-border bg-card/70 p-4">
        <div className="text-xs font-bold text-muted-foreground mb-2">{t("STEP 1　幹線サイズを選ぶ（mm²）")}</div>
      ...
      {main != null && (
        <div className="rounded-2xl border border-border bg-card/70 p-4">
          <div className="text-xs font-bold text-muted-foreground mb-2">{t("STEP 2　分岐サイズを選ぶ（mm²）")}</div>
      ...
      {main != null && branch != null && total != null && (
        <div className="rounded-2xl border border-border bg-card/70 p-4">
          {match ? (
            <>
              <div className="text-xs font-bold text-muted-foreground">{t("使用するT形コネクタ")}</div>
              <div className="mt-1 text-5xl font-black leading-tight tabular-nums" style={{ color: "#03AF7A" }}>
                {match.name}
              </div>
              <div className="mt-2 text-sm">
                {t("適用範囲：")}{match.min}〜{match.max} mm²
              </div>
            </>
          ) : (
            <>
              <div className="text-xs font-bold text-muted-foreground">{t("判定")}</div>
              <div className="mt-1 text-4xl font-black leading-tight" style={{ color: "#FF4B00" }}>
                {t("範囲外")}
              </div>
              <div className="mt-2 text-sm">
                {t("T形コネクタの適用範囲を超えています。電線の組み合わせを見直してください。")}
              </div>
            </>
          )}
          <div className="mt-2 text-[11px] text-muted-foreground">
            {t("幹線")} {mainWire?.label}mm² ＋ {t("分岐")} {branchWire?.label}mm² ＝ {t("合計")} {total} mm²
          </div>
        </div>
      )}

      <div className="text-[11px] text-muted-foreground px-1 space-y-0.5">
        <p>{t("※ ニチフ T形コネクタ（分岐接続用）カタログ準拠。")}</p>
        <p>{t("※ 適用電線断面積の合計から号数を選定します。")}</p>
      </div>
    </div>
  );
}
...
function CounterTab({
  ...
  return (
    <div className="space-y-4">
      {/* 電線カウンター */}
      <div className="rounded-2xl border border-border bg-card/70 p-4">
        <div className="text-xs font-bold text-muted-foreground mb-2">{t("電線をタップして本数を追加（10回目で1本に戻る）")}</div>
        <div className={`grid gap-2 ${cols === 3 ? "grid-cols-3" : cols === 5 ? "grid-cols-5" : "grid-cols-4"}`}>
          {wires.map((w, i) => {
            const n = counts[i] ?? 0;
            return (
...
function PResult({ lang, total }: { lang: GenbaLang; total: number }) {
  const t = (ja: string) => genbaTr(ja, lang);
  ...
  return (
    <div className="rounded-2xl border border-border bg-card/70 p-4">
      {match ? (
        <>
          <div className="text-xs font-bold text-muted-foreground">{t("使用するP形スリーブ")}</div>
          <div className="mt-1 text-5xl font-black leading-tight tabular-nums" style={{ color: "#03AF7A" }}>
            {match.name}
          </div>
          <div className="mt-2 text-sm">
            {t("抱合範囲：")}{match.min}〜{match.max} mm²
          </div>
        </>
      ) : (
        <>
          ...
          <div className="mt-2 text-sm">{t("Pスリーブの抱合範囲を超えています。電線の組み合わせを見直してください。")}</div>
        </>
      )}
      <div className="mt-2 text-[11px] text-muted-foreground">{t("合計")} {total.toFixed(2)} mm²</div>
    </div>
  );
}

function RingResult({ lang, n16, n20, n26 }: { lang: GenbaLang; n16: number; n20: number; n26: number }) {
  const t = (ja: string) => genbaTr(ja, lang);
  const judge = judgeRingSleeve(n16, n20, n26);
  if (judge == null) {
    return (
      <div className="rounded-2xl border border-border bg-card/70 p-4">
        <div className="text-sm text-muted-foreground">{t("電線を合計2本以上追加すると判定します。")}</div>
      </div>
    );
  }
      ...
      {judge.kind === "ok" ? (
        <>
          <div className="grid grid-cols-2 gap-2">
            <div>
              <div className="text-xs font-bold text-muted-foreground">{t("スリーブサイズ")}</div>
              <div className="mt-1 text-5xl font-black leading-tight" style={{ color: "#03AF7A" }}>
                {judge.size}
              </div>
            </div>
            <div>
              <div className="text-xs font-bold text-muted-foreground">{t("圧着マーク")}</div>
              <div className="mt-1 text-5xl font-black leading-tight" style={{ color: "#4DC4FF" }}>
                {judge.mark}
              </div>
            </div>
          </div>
          <div className="mt-2 text-[11px] text-muted-foreground">{combo}{t("の接続。")}</div>
        </>
      ) : (
        <>
          <div className="text-xs font-bold text-muted-foreground">{t("判定")}</div>
          ...
          <div className="mt-2 text-sm">{t("組み合わせ範囲外です。電線の組み合わせを見直してください。")}</div>
...
function CEResult({ lang, total }: { lang: GenbaLang; total: number }) {
  const t = (ja: string) => genbaTr(ja, lang);
  ...
  return (
    <div className="rounded-2xl border border-border bg-card/70 p-4">
      {matches.length > 0 ? (
        <>
          <div className="text-xs font-bold text-muted-foreground">{t("使用するCE形スリーブ（候補）")}</div>
        ...
        <>
          ...
          <div className="mt-2 text-sm">{t("CE形スリーブの抱合範囲を超えています。電線の組み合わせを見直してください。")}</div>
        </>
      )}
      <div className="mt-2 text-[11px] text-muted-foreground">{t("合計")} {total.toFixed(2)} mm²</div>
    </div>
