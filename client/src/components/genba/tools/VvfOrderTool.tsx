...
function RemainBadge({ remain }: { remain: number }) {
  if (remain === 0) {
    return <span className="text-[11px] font-bold" style={{ color: "#03AF7A" }}>ちょうど</span>;
  }
  return <span className="text-[11px] text-muted-foreground">余り {remain}m</span>;
}
 ...
 */
export default function VvfOrderTool() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-base font-bold">VVF 発注計算</h2>
      </div>
      <p className="text-xs text-muted-foreground px-1 -mt-2">
        回路ごとに長さと回路数を入力すると、ロス率込みの発注数量を即時計算します。
      </p>

      {/* STEP 1: ロス率 */}
      <div className="rounded-2xl border border-border bg-card/70 p-4">
        <div className="text-xs font-bold text-muted-foreground mb-2">STEP 1　ロス率を設定</div>
        <div className="flex items-center gap-2">
          <input
            type="number"
            inputMode="decimal"
          ...
          <span className="text-[11px] text-muted-foreground">入力長さに上乗せして計算します（{VVF_LOSS_MIN}〜{VVF_LOSS_MAX}%）</span>
        </div>
      </div>

      {/* STEP 2: 回路入力 */}
      <div className="rounded-2xl border border-border bg-card/70 p-4 space-y-3">
        <div className="text-xs font-bold text-muted-foreground">STEP 2　回路を入力</div>
        {rows.map((r, i) => (
          <div
            key={r.id}
          ...
          >
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-bold text-muted-foreground">回路 {i + 1}</span>
              {rows.length > 1 && (
                <button
                  type="button"
                  aria-label="この回路を削除"
            ...
            <div className="grid grid-cols-2 gap-2">
              <label className="space-y-1">
                ...
                >
                  {VVF_SIZES.map((s) => (
                    <option key={s} value={s}>VVF {s}</option>
                  ))}
                </select>
              </label>
              <label className="space-y-1">
                <span className="text-[11px] text-muted-foreground">種別</span>
                <select
                  value={r.lineType}
                  onChange={(e) => patch(r.id, { lineType: e.target.value as VvfLineType })}
                  className="w-full rounded-xl border border-border bg-background px-2 py-2 text-sm font-bold"
                >
                  {VVF_LINE_TYPES.map((t) => (
                    <option key={t.key} value={t.key}>{t.key}</option>
                  ))}
                </select>
              </label>
              <label className="space-y-1">
                <span className="text-[11px] text-muted-foreground">長さ（m/回路）</span>
                <input
                  type="number"
                  inputMode="decimal"
                  min={0}
              ...
              <label className="space-y-1">
                <span className="text-[11px] text-muted-foreground">回路数</span>
                <input
                  type="number"
                  inputMode="numeric"
                  min={1}
          ...
          {results.map((g) => (
            ...
            >
              <div className="text-sm font-bold" style={{ color: VVF_SIZE_COLORS[g.size] }}>
                VVF {g.size}　{g.lineType}
                <span className="ml-1 text-[11px] font-normal text-muted-foreground">（{g.colors}）</span>
              </div>
              <div className="mt-1 text-[11px] text-muted-foreground">
                入力合計 {g.rawTotalM}m → ロス込み <span className="font-bold text-foreground">{g.withLossM}m</span>
              </div>
              <div className="mt-2 grid grid-cols-2 gap-2">
                <div className="rounded-xl border border-border p-2.5 text-center">
                  <div className="text-[11px] text-muted-foreground">{VVF_ROLL_M}m巻</div>
                  <div className="text-2xl font-bold leading-tight" style={{ color: "#4DC4FF" }}>
                    {g.rolls100}<span className="text-sm">巻</span>
                  </div>
                  <RemainBadge remain={g.remain100} />
                </div>
                <div className="rounded-xl border border-border p-2.5 text-center">
                  <div className="text-[11px] text-muted-foreground">{VVF_DRUM_M}mドラム</div>
                  <div className="text-2xl font-bold leading-tight">
                    {g.drums500}<span className="text-sm">本</span>
                  </div>
                  <RemainBadge remain={g.remain500} />
                </div>
              </div>
            </div>
          ))}
      ...
      <p className="text-[11px] text-muted-foreground px-1">
        ※ ロス率・梱包単位（{VVF_ROLL_M}m巻 / {VVF_DRUM_M}mドラム）は一般的な目安です。実際の発注は納入仕様・メーカーの販売単位を確認してください。
      </p>
    </div>
  );
}
