 ...
 */
export default function PipeOuterDiameterTool() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-base font-bold">配管外径 早見</h2>
        <button
          type="button"
      ...
      <div className="rounded-2xl border border-border bg-card/70 p-4">
        <div className="text-xs font-bold text-muted-foreground mb-2">STEP 1　配管の種類</div>
      ...
      {listMode && pipe && (
        <div className="rounded-2xl border border-border bg-card/70 p-4">
          <div className="text-xs font-bold text-muted-foreground mb-2">
            {pipe.label}（{pipe.sub}）呼び径 × 外径 一覧
          </div>
          <table className="w-full text-sm tabular-nums">
            <thead>
              <tr className="text-xs text-muted-foreground border-b border-border">
                <th className="text-left font-bold py-1.5">呼び径</th>
                <th className="text-right font-bold py-1.5">外径 (mm)</th>
              </tr>
            </thead>
            <tbody>
              {sizes.map((s) => (
          ...
          <div className="mt-2 text-[11px] text-muted-foreground">
            {pipe.approx ? "※ カタログ参照の目安値（メーカーにより異なります）" : "※ JIS C 8305 規格値"}
          </div>
        </div>
      )}
      {listMode && !pipe && (
        <p className="text-sm text-muted-foreground text-center py-2">
          一覧表を見るには配管の種類を選んでください。
        </p>
      )}
      ...
      {pipe && size != null && od != null && (
        <div className="rounded-2xl border border-border bg-card/70 p-4">
          <div className="text-xs text-muted-foreground">
            {pipe.label}（{pipe.sub}）呼び径 {size} の外径
          </div>
          ...
          <div className="mt-2 text-[11px] text-muted-foreground">
            {pipe.approx
              ? "※ カタログ参照の目安値です。メーカーにより外径が異なる場合があります。"
              : "※ JIS C 8305 規格値です。"}
          </div>
        </div>
      )}
      ...
      <p className="text-[11px] text-muted-foreground px-1">
        ※ 本ツールの数値は規格・カタログに基づく目安です。実施工では現場実測・設計図書を優先してください。
      </p>
    </div>
  );
}
