...
export default function WorkerLinksPanel({
  ...
  const issue = trpc.genba.workerLinks.issue.useMutation({
    onSuccess: async (l) => {
      invalidate();
      if (l?.token) { await copyUrl(l.token); toast.success(t("リンクを発行してコピーしました")); }
    },
  ...
  const remove = trpc.genba.workerLinks.remove.useMutation({ onSuccess: () => { invalidate(); toast.success(t("リンクを削除しました")); }, onError: (e) => toast.error(e.message) });
  const setGenbaRole = trpc.genba.users.setGenbaRole.useMutation({
    onSuccess: () => { utils.genba.users.siteRoster.invalidate({ siteId }); toast.success(t("権限を変更しました")); },
    onError: (e) => toast.error(e.message),
  });
  const setWorkerRole = trpc.genba.workerLinks.setWorkerRole.useMutation({
    onSuccess: () => { utils.genba.users.siteRoster.invalidate({ siteId }); invalidate(); toast.success(t("権限を変更しました")); },
    onError: (e) => toast.error(e.message),
  });
  const renameWorker = trpc.genba.workerLinks.renameWorker.useMutation({
    onSuccess: () => { utils.genba.users.siteRoster.invalidate({ siteId }); invalidate(); toast.success(t("名前を修正しました")); },
    onError: (e) => toast.error(e.message),
  });
  const promptRename = (siteWorkerId: string, current: string) => {
    const v = window.prompt(t("ゲストの表示名を修正"), current);
  ...
  const deleteWorker = trpc.genba.workerLinks.deleteWorker.useMutation({
    onSuccess: () => { utils.genba.users.siteRoster.invalidate({ siteId }); invalidate(); toast.success(t("名簿から削除しました")); },
    onError: (e) => toast.error(e.message),
  });
  const promptDeleteWorker = (siteWorkerId: string, name: string) => {
    if (window.confirm(`${name}${t(" を名簿から完全に削除しますか？\n（専用リンクと、この作業員の全作業への割当も一緒に消えます）")}`)) deleteWorker.mutate({ siteWorkerId });
  };
  ...
  async function copyUrl(token: string) {
    ...
    try { await navigator.clipboard.writeText(url); } catch { window.prompt(t("コピーしてください"), url); }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader><DialogTitle>🔗 {t("作業員リンク管理")}</DialogTitle></DialogHeader>
        <p className="text-xs text-muted-foreground">
          {t("作業員ごとの専用リンクです。ゲストや外注の方は")}<strong>{t("ログイン不要")}</strong>{t("でこのリンクから自分の担当作業を確認・更新できます。現場から外れた人はここで")}<strong>{t("無効化")}</strong>{t("してください（再有効化・再発行も可能）。")}
        </p>
        <div className="flex items-center gap-2 text-xs">
          <span className="text-muted-foreground">{t("新規発行の有効期限:")}</span>
          <select value={expiresDays} onChange={(e) => setExpiresDays(e.target.value)} className="rounded-md border border-border bg-background p-1.5 text-xs">
            <option value="">{t("無期限")}</option>
            <option value="7">{t("7日")}</option>
            <option value="30">{t("30日")}</option>
            <option value="90">{t("90日")}</option>
          </select>
        </div>

        {/* 発行済みリンク一覧 */}
        <div className="rounded-lg border border-border divide-y divide-border/60">
          <div className="px-2 py-1.5 text-[11px] font-bold text-muted-foreground bg-muted/40">{t("発行済みリンク")}（{linkList.length}）</div>
          {linkList.length === 0 && <p className="text-sm text-muted-foreground p-3">{t("まだ発行されていません。下の名簿から発行してください。")}</p>}
          {linkList.map((l) => {
            ...
            return (
              <div key={l.id} className="p-2 space-y-1.5">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {l.kind === "guest" && (
                    <button title={t("ゲスト名を修正")} className="text-muted-foreground hover:text-foreground" onClick={() => promptRename(l.siteWorkerId, l.displayName)}>
                      <Pencil className="h-3 w-3" />
                    </button>
                  )}
                  {l.kind === "guest" && (
                    <button title={t("名簿から完全に削除（リンク・割当も消去）")} className="text-muted-foreground hover:text-destructive" onClick={() => promptDeleteWorker(l.siteWorkerId, l.displayName)}>
                  ...
                  <select value={l.role} disabled={!isAdmin || setRole.isPending}
                    title={isAdmin ? t("このリンクの権限") : t("権限を変更できるのは管理者のみです")}
                    ...
                    <option value="worker">{t("作業員")}</option>
                    <option value="leader">{t("リーダー")}</option>
                  </select>
                  {l.active && !expired
                    ? <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#03AF7A] text-white font-bold">{t("有効")}</span>
                    : expired
                      ? <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#F6AA00] text-[#3a2a00] font-bold">{t("期限切れ")}</span>
                      : <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#84919E] text-white font-bold">{t("無効化済み")}</span>}
                  <span className="ml-auto text-[10px] text-muted-foreground">{t("最終アクセス")} {fmt(l.lastAccessAt)}{l.expiresAt ? ` / ${t("期限")} ${fmt(l.expiresAt)}` : ""}</span>
                </div>
                <div className="flex gap-1.5 flex-wrap">
                  <Button size="sm" variant="outline" className="h-7 text-xs" onClick={async () => { await copyUrl(l.token); toast.success(t("リンクをコピーしました")); }}>
                    <Copy className="h-3 w-3 mr-1" /> {t("コピー")}
                  </Button>
                  <Button size="sm" variant="outline" className="h-7 text-xs"
                    onClick={() => { if (window.confirm(t("再発行すると今のリンクは使えなくなります。よろしいですか？"))) issue.mutate({ siteWorkerId: l.siteWorkerId, role: l.role, expiresDays: expiryOpt }); }}>
                    <RefreshCw className="h-3 w-3 mr-1" /> {t("再発行")}
                  </Button>
                  {l.active ? (
                    <Button size="sm" variant="outline" className="h-7 text-xs text-[#b45309]"
                      onClick={() => setActive.mutate({ id: l.id, active: false })}>
                      <Ban className="h-3 w-3 mr-1" /> {t("無効化")}
                    </Button>
                  ) : (
                    <Button size="sm" variant="outline" className="h-7 text-xs text-[#03AF7A]"
                      onClick={() => setActive.mutate({ id: l.id, active: true })}>
                      <CircleCheck className="h-3 w-3 mr-1" /> {t("有効化")}
                    </Button>
                  )}
                  <Button size="sm" variant="ghost" className="h-7 text-xs text-destructive ml-auto"
                    onClick={() => { if (window.confirm(`${l.displayName}${t(" のリンクを完全に削除しますか？")}`)) remove.mutate({ id: l.id }); }}>
                    <Trash2 className="h-3 w-3 mr-1" /> {t("削除")}
                  </Button>
                </div>
              </div>
            );
        ...
        <div className="rounded-lg border border-border divide-y divide-border/60">
          <div className="px-2 py-1.5 text-[11px] font-bold text-muted-foreground bg-muted/40">
            {t("名簿（リンク未発行）")}{linked ? ` — ${t("出面に登録された人")}` : ""}
          </div>
          {rosterWithoutLink.length === 0 && <p className="text-sm text-muted-foreground p-3">{roster.length === 0 ? t("名簿が空です。案件連携と出面の登録を確認してください。") : t("全員に発行済みです。")}</p>}
            ...
            return (
              <div key={r.siteWorkerId} className="p-2 flex items-center gap-2 flex-wrap">
                ...
                {r.kind === "guest" && r.siteWorkerId && (
                  <button title={t("ゲスト名を修正")} className="text-muted-foreground hover:text-foreground" onClick={() => promptRename(r.siteWorkerId as string, r.displayName)}>
                    <Pencil className="h-3 w-3" />
                  </button>
                )}
                {r.kind === "guest" && r.siteWorkerId && (
                  <button title={t("名簿から完全に削除（割当も消去）")} className="text-muted-foreground hover:text-destructive" onClick={() => promptDeleteWorker(r.siteWorkerId as string, r.displayName)}>
                    <UserX className="h-3.5 w-3.5" />
                  </button>
                )}
                <span className={`text-[9px] px-1 py-0.5 rounded border leading-none ${kind.cls}`}>{kind.label}</span>
                ...
                {isOwner ? (
                  ...
                  <select value={(r as any).workerRole ?? "worker"}
                    ...
                    title={isAdmin ? t("この現場での役割 (リンク発行時の権限になります)") : t("権限を変更できるのは管理者のみです")}
                ...
                {(r as any).roleOverridden && <span title={t("genba内で上書きされた権限")} className="text-[10px] text-[#005AFF] font-bold">*</span>}
                <Button size="sm" variant="outline" className="h-7 text-xs ml-auto"
                  ...
                  <LinkIcon className="h-3 w-3 mr-1" /> {t("専用リンクを発行")}
                </Button>
              </div>
            );
          })}
