...
export default function TeamManager({ siteId, open, onOpenChange }: { siteId: string; open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  const create = trpc.genba.teams.create.useMutation({ onSuccess: () => { refresh(); toast.success("班を作成しました"); }, onError: (e) => toast.error(e.message) });
  const rename = trpc.genba.teams.rename.useMutation({ onSuccess: refresh, onError: (e) => toast.error(e.message) });
  const remove = trpc.genba.teams.remove.useMutation({ onSuccess: () => { refresh(); toast.success("班を削除しました"); }, onError: (e) => toast.error(e.message) });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>班の管理</DialogTitle>
        </DialogHeader>

        <div className="space-y-4">
          {teamList.length === 0 && <p className="text-sm text-muted-foreground">班がありません。下のボタンから作成してください。</p>}
          {teamList.map((g) => (
            <div key={g.id} className="rounded-lg border border-border p-2 space-y-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground">{g.memberIds.length}名</span>
                <Button variant="ghost" size="sm" className="px-1 h-7" onClick={() => { const v = window.prompt("班名を変更", g.name); if (v && v.trim() && v.trim() !== g.name) rename.mutate({ id: g.id, name: v.trim() }); }}>
                  <Pencil className="h-3.5 w-3.5" />
                </Button>
                <Button variant="ghost" size="sm" className="px-1 h-7 text-destructive hover:text-destructive" onClick={() => { if (confirm(`「${g.name}」を削除しますか？`)) remove.mutate({ id: g.id }); }}>
          ...
          <Button variant="outline" size="sm" className="w-full" onClick={() => { const v = window.prompt("班名を入力", `${teamList.length + 1}班`); if (v && v.trim()) create.mutate({ siteId, name: v.trim() }); }}>
            <Plus className="h-4 w-4 mr-1" /> 班を作成
          </Button>
        </div>
      </DialogContent>
    </Dialog>
