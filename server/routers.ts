...
export const appRouter = router({
  ...
  superAdmin: router({
    listPasswordRecoveryRequests: superAdminProcedure.query(async () => {
      ...
      const employeeIds = Array.from(new Set(requests.map((request) => request.employeeId).filter((id): id is number => typeof id === "number")));
      const userIds = Array.from(new Set(requests.map((request) => request.userId).filter((id): id is number => typeof id === "number")));

      const [employees, users] = await Promise.all([
        employeeIds.length
          ? dbInstance
              .select({ id: schema.employees.id, nameKanji: schema.employees.nameKanji, nameRomaji: schema.employees.nameRomaji })
              .from(schema.employees)
              .where(inArray(schema.employees.id, employeeIds))
          : Promise.resolve([]),
        userIds.length
          ? dbInstance
              .select({ id: schema.users.id, appRole: schema.users.appRole })
              .from(schema.users)
