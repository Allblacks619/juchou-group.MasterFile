...
export const appRouter = router({
  ...
  workerInvoice: router({
    getMyDraft: protectedProcedure.input(z.object({ projectId: z.number(), closingMonth: z.string().regex(/^\d{4}-\d{2}$/) })).query(async ({ ctx, input }) => {
      ...
      // 保存済み明細が無ければ、V2から自動計算した明細（出勤日数×単価＋交通費）を提案として返す。
      // 作業員は締め提出前でも下書きしたいので、提出ゲートはバイパスする。
      if (items.length === 0) {
        try {
          ...
          if (autoItems.length > 0) {
            return { ...invoice, items: autoItems, autoGenerated: true, warnings: draft.warnings };
          }
        } catch {
          /* 自動生成できない場合（単価未設定・出面なし等）は空のまま返す */
        }
      }
