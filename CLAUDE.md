...
## ホームページ再構築（2026-07 進行中）
- 状態: G4b「複線図から現場へ」が最新候補。**オーナー承認待ち。承認なしに本番反映しないこと。**
- 全モックアップ・設計資料: `docs/homepage-redesign/`（HTML + `specs/` ディレクトリ）
- 引き継ぎ詳細: `docs/引き継ぎ.md`（決定理由・却下履歴・事実ルール・次ステップ）
- 事実の正本: `docs/homepage-redesign/specs/JP-DENKI-FACTS.md`（日本の電気工法。ビジュアルがこれに反したら却下）
- 技術制約: 自己完結HTML / 外部リクエスト禁止 / Canvas 2D生成 / rAF+transformパララックス / writing-mode禁止 / prefers-reduced-motion対応
- **絶対ルール: オーナーが「これを出して」と明言するまで本番デプロイしない**（過去に無断デプロイでインシデント発生）
